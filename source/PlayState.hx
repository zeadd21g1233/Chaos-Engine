...
class PlayState extends MusicBeatState
{
	...
	var ChaosEngineWatermark:FlxText;
	...
	private var executeModchart = false;
        ...
        var HScriptModchart:HxModchart;
        var Usingdata:String = "";
	public var foreground:FlxSpriteGroup; // Its For Script
	...
	override public function create()
	{
		...
		instance = this;
                ...
                NoteSplash.IsPixelSplash = false;
		...
		switch(stageCheck)
		{
			...
			case 'schoolEvil':
			{
					...
					/* 
						...
						*/
                        ...
                        case 'tank': //Week 7 - Ugh, Guns, Stress --Shadow Mario Please Dont Kill ME-
                        {
			curStage = 'tank';
			defaultCamZoom = 0.9;
			...
			moveTank();
                        ...
                        tank0 = new BGSprite('tank0', -500, 650, 1.7, 1.5);
                        tank1 = new BGSprite('tank1', -300, 750, 2, 0.2);
                        tank2 = new BGSprite('tank2', 450, 940, 1.5, 1.5);
                        tank4 = new BGSprite('tank4', 1300, 900, 1.5, 1.5);
                        tank5 = new BGSprite('tank5', 1620, 700, 1.5, 1.5);
                        tank3 = new BGSprite('tank3', 1300, 1200, 3.5, 2.5);
		...
		boyfriend = new Boyfriend(770, 450, SONG.player1);
                ...
                var GFBFDead:Boyfriend;
                GFBFDead = new Boyfriend(10000, 10000, 'bf-holding-gf-dead'); // Preload Death Anim
                add(GFBFDead);
		...
		// Add Chaos Engine watermark
		ChaosEngineWatermark = new FlxText(1070,healthBarBG.y + 50,0,SONG.song + " - " + CoolUtil.difficultyFromInt(storyDifficulty) + (Main.watermarks ? " | CE " + MainMenuState.chaosEngineVer : ""), 16);
		ChaosEngineWatermark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		ChaosEngineWatermark.scrollFactor.set();
		add(ChaosEngineWatermark);

		if (PlayStateChangeables.useDownscroll)
			ChaosEngineWatermark.y = FlxG.height * 0.9 + 45;
		...
		ChaosEngineWatermark.cameras = [camHUD];
		if (loadRep)
		...
		FlxG.stage.addEventListener(KeyboardEvent.KEY_DOWN,handleInput);
	        ...
	        if (Usingdata != "")
	        {
                ...
                HScriptModchart.ScriptCreatePost();
	...
	function startSong():Void
	{
		...
		if (useVideo)
	        ...
	        if (Usingdata != "")
	        {
                HScriptModchart.ScriptSongStart();
                }
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		{
			// FlxG.log.add(i);
			var babyArrow:FlxSprite = new FlxSprite(PlayStateChangeables.useMiddlescroll ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLine.y);
			...
			{
				case 'pixel':
					babyArrow.loadGraphic(Paths.image('NoteSprites/Note/pixel/arrows-pixels', 'shared'), true, 17, 17);
					...
					case 'normal':
						babyArrow.frames = Paths.getSparrowAtlas('NoteSprites/Note/Default/NOTE_assets');
	...
	override public function update(elapsed:Float)
	{
		...
		switch (curStage)
		{
			case 'tank':
                ...
                if (PlayStateChangeables.useMiddlescroll == true) //Iknow This Fucking Shit Code
                {
                        cpuStrums.forEach(function(spr:FlxSprite)
                        {
                        spr.alpha = 0;
		...
		if (executeModchart && luaModchart != null && songStarted)
		{
			...
			if (luaModchart.getVar("showOnlyStrums",'bool'))
			{
				...
				ChaosEngineWatermark.visible = false;
			...
			else
			{
				...
				ChaosEngineWatermark.visible = true;
		...
		scoreTxt.x = (originalX - (lengthInPx / 2)) + 350;
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
	                                        ...
	                                        if (Usingdata != "")
	                                        {
                                                HScriptModchart.ScriptopponentNoteHit(daNote);
                                                }
						...
						{
							case 2:
                                                        if (daNote.noteStyle != 'Alt')
								targ.playAnim('singUP' + altAnim, true);

                                                                if (daNote.noteStyle == 'Alt')
                                                                {
                                                                targ.playAnim('singUP-alt', true);
                                                                }
							case 3:
                                                        if (daNote.noteStyle != 'Alt')
								targ.playAnim('singRIGHT' + altAnim, true);
                                                                ...
                                                                if (daNote.noteStyle == 'Alt')
                                                                {
                                                                targ.playAnim('singRIGHT-alt', true);
                                                                }
							case 1:
                                                        if (daNote.noteStyle != 'Alt')
								targ.playAnim('singDOWN' + altAnim, true);
                                                                ...
                                                                if (daNote.noteStyle == 'Alt')
                                                                {
                                                                targ.playAnim('singDOWN-alt', true);
                                                                }
							case 0:
                                                        if (daNote.noteStyle != 'Alt')
								targ.playAnim('singLEFT' + altAnim, true);
                                                                ...
                                                                if (daNote.noteStyle == 'Alt')
                                                                {
                                                                targ.playAnim('singLEFT-alt', true);
                                                                }
						}
		...
		if (FlxG.keys.justPressed.ONE)
	        ...
	        if (Usingdata != "")
	        {
                HScriptModchart.ScriptUpdateConstant(elapsed);
                }
	...
	*/
	...
	override function stepHit()
	{
		...
		if (executeModchart && luaModchart != null)
		{
	        ...
	        if (Usingdata != "")
            	{
		HScriptModchart.ScriptUpdateStep(curStep);
                }
	...
	override function beatHit()
	{
		...
		if (executeModchart && luaModchart != null)
		{
			...
			luaModchart.executeState('beatHit',[curBeat]);
                ...
                HScriptModchart.ScriptBeat(curBeat);
	...
	var curLight:Int = 0;
        ...
        public function CamZoom(Found:Int) // For Some Reason camera zoom didnt work in script
        {
        defaultCamZoom = Found;
        }



        public function HScriptStart()
        {

        var formattedFolder:String = Paths.formatToSongPath(SONG.song);
        
        var path:String = Paths.hscript(formattedFolder + '/script'); //data/dad-battle/script.hxs 

	if (FileSystem.exists(path))
		Usingdata = File.getContent(path);

	if (Usingdata != "")
	{
	HScriptModchart = new HxModchart();
	add(HScriptModchart); 

        var exposure:StringMap<Dynamic> = new StringMap<Dynamic>();

        // playstate
        exposure.set('difficulty', storyDifficulty);
        exposure.set('daPixelZoom', daPixelZoom);
        exposure.set('isStoryMode', isStoryMode);
        exposure.set('isHalloween', isHalloween);
        exposure.set('generatedMusic', generatedMusic);
        exposure.set('startedCountdown', startedCountdown);
        exposure.set('defaultCamZoom', defaultCamZoom);
        exposure.set('health', PlayState.instance.health);

        //Camera
        exposure.set('camHUD', camHUD);
        exposure.set('camGame', camGame);
        exposure.set('camMisc', camMisc);
        exposure.set('camFollow', camFollow);

        //song
        exposure.set('bpm', Conductor.bpm);
        exposure.set('songPosition', Conductor.songPosition);
        exposure.set("song", SONG.song);
        exposure.set("songLowercase", SONG.song.replace(" ", "-").trim().toLowerCase());
        exposure.set("songData", SONG);
        exposure.set("SONG", SONG);
	exposure.set("curStep", curStep);
	exposure.set("curBeat", curBeat);
	exposure.set("songStarted", songStarted);
	...
	exposure.set('gf', gf);
        ...
        //Note Stuff
        exposure.set('strumLineNotes', strumLineNotes);
        exposure.set('playerStrums', playerStrums);
        exposure.set('cpuStrums', cpuStrums);     
        exposure.set('notes', notes);
        exposure.set('unspawnNotes', unspawnNotes);


        HxModchart.HScriptsModchart = ScriptHandler.loadModule('data/$formattedFolder/script', exposure);
	if (HxModchart.HScriptsModchart.exists("onCreate"))
		HxModchart.HScriptsModchart.get("onCreate")();
        //Paths.revertCurrentLevel();
        //Paths.setCurrentLevel('assets/data/$formattedFolder');

        trace('$formattedFolder loaded Script successfully');
